...
GameController::~GameController() {
	...
	// OBSERVER PATTERN: Delete mapView
	delete mapDisplay;
	mapDisplay = nullptr;
}
...
void GameController::initializeNewGame()
{
	...
	addEpidemicCards();
	initialInfection();
	mapDisplay->Update(); // Force Update for initial map display
...
void GameController::phase1_Actions()
{
	...
	while(i < NUM_OF_ACTIONS) {
		...
		// Display player view
		playerDisplay.printPlayerDisplay(players[activePlayer]);  
		...
		if (action.execute(map, &markers, players, playerDeck, infectionDeck, activePlayer)) {
			i++;
		}
...
void GameController::phase2_DrawCards()
{
	...
	if (playerDeck->cardsInDeck() < 2) {
		...
		for (int i = 0; i < NUM_OF_PLAYER_CARDS_DRAWN_PER_TURN; i++) {
			...
			action.execute(map, &markers, players, playerDeck, infectionDeck, activePlayer);
		}
	}
...
void GameController::phase3_Infects()
{
	...
	if (!quietNightPlayed) {
		...
		cout << "You had a quiet night, no new disease cubes placed" << endl;
	}
}
